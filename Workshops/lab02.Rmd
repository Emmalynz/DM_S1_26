...
Today we'll use a new data set for schools alongside the school roll data, and some new data from the Ministry of Health. We'll start by loading in the roll and schools data, along with loading the `readxl` package for reading data directly from an excel spreadsheet:
...
## Try yourself
ethnicity_bylevel <- clean |> group_by(ethnicity) |> summarise(total = sum(students))
...
ggplot(ethnicity_by_year) +
  geom_col(mapping = aes(x = Level, y = Total)) +
  facet_wrap(vars(EthnicGroup), scales = "free")
...
#3. Trying 2. but so other is at the end

ethnic_levels <- c("Asian", "European", "Māori", "MELAA", "Pacific", "Other")

ethnicity_by_year |> 
  filter(EthnicGroup != "International fee paying") |> 
  mutate(EthnicGroup = factor(EthnicGroup, levels = ethnic_levels)) |> 
  ggplot() +
  geom_col(mapping = aes(x = Level, y = Total)) +
  facet_wrap(vars(EthnicGroup), scales = "free")
...
```{r}
#1. How many schools are there in the Palmerston North Territorial Authority? There are 43 schools within the Palmerston North Territorial Authority
unique(schools$TerritorialAuthority)
...
schools |>  filter(Sector == "Secondary") |>
  ...
  group_by(SchoolGender, Takiwa) |>
  summarise(count = n()) |>
  
  ggplot(aes(x = SchoolGender, y = count, fill = SchoolGender)) +
    geom_col() +
     facet_wrap(vars(Takiwa), ncol= 1)+
      labs(title = "Secondary Schools by School Gender " ) +
...
all_data |> group_by(EthnicGroup, Takiwa) |>
...
all_data |>
  filter(EthnicGroup != "International fee paying") |>
  group_by(Takiwa, AffiliationType, Level) |>
  summarise(Total = sum(Students)) |>
  ggplot() +
  geom_col(mapping=aes(x=Level, y=Total)) +
  facet_grid(rows = vars(Takiwa),
             cols = vars(AffiliationType),
             scales = "free_x")
...
# 2. Create a table with the number of students in each ethnic group for each year level.

 all_data|> group_by(Level, EthnicGroup) |>
  summarise(Total = sum(Students)) |>
...
ethnicity_by_takiwa |> group_by(Takiwa, EthnicGroup) |>
  pivot_wider(names_from = EthnicGroup, values_from = Students)|> 
  mutate(Total = 'Te Tai Raro', 'Te Tai Runga', 'Te Tai Whenua')

#or - more reliable to changes
...
all_data |> group_by(Takiwa, EthnicGroup) |>
  ...
  pivot_wider(names_from = Takiwa, values_from = Students) |>
  relocate(Total, .after = last_col()) #move the total column to the last col

#or doesnt use double group by - adds complexity, mutate is taking place within ethnic group.
...
all_data |>
  group_by(Takiwa, EthnicGroup) |>
  summarise(Students = sum(Students))|>
   mutate(Percentage = round(Students/sum(Students) *100, digits = 1)) |> 
  pivot_wider(names_from = Takiwa, values_from = Percentage)
 #notice graph has NAs not quite the result we wish, soemting has been bought over...

all_data |>
  group_by(Takiwa, EthnicGroup) |>
  summarise(Students = sum(Students))|>
   mutate(Percentage = round(Students/sum(Students) *100, digits = 1)) |> 
  select(-Students) |> #remove so pivoting cn compress rows
  pivot_wider(names_from = Takiwa, values_from = Percentage)
...
#this is a bar plot for regional vaccination counts by milestone in a facet wrap with a line and colour markings differentiating DHBs that reach 90% vaccination for that age group.


imm_clean |>
  filter(Milestone != "5 years") |>
  mutate(Milestone = fct_relevel(Milestone, "6 months", "8 months")) |>
  mutate(MetTarget = Proportion > 0.9)|>
  mutate(DHB = fct_rev(DHB)) |>
  ggplot() +
  aes(x = Proportion, y = DHB,fill = MetTarget) + 
  geom_vline(xintercept = 0.9, alpha = 0.7) +
  geom_col(alpha = 0.8) +
  facet_wrap(vars(Milestone)) +
  scale_fill_manual(values = c("#C582B2", "#B7B7B2"), guide = "none")+
